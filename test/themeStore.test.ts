...
const originalConfigDir = process.env.GLABUI_CONFIG_DIR
const tempDirs: string[] = []

const restoreConfigDir = () => {
	if (originalConfigDir === undefined) {
		delete process.env.GLABUI_CONFIG_DIR
	} else {
		process.env.GLABUI_CONFIG_DIR = originalConfigDir
	}
}
...
const useTempConfig = async (content?: string) => {
	const dir = await mkdtemp(join(tmpdir(), "glabui-theme-store-"))
	tempDirs.push(dir)
	process.env.GLABUI_CONFIG_DIR = dir
